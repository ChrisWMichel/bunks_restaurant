        ...
        methods:{
            getAdmins(){
                axios.get('api/get_admins').then((resp) => {
                    this.$store.dispatch('uploadAdmins', resp.data);
            ...
            RegisterNewAdmin(){
                axios.post('/notify-admin', this.form)
                    .then(response => {
                        ...
                        this.form.lastname = '';
                        this.form.email = '';
                        toastr.success('An email has been sent ' + firstname +' to complete the registration.');
